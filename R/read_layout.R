...
read_layout <- function(filepath) {
  ...
  out %>%
    purrr::set_names( c("variable", "row", .data$.[1,][-c(1,2)])) %>%
    dplyr::filter(row %in% base::LETTERS[1:16]) %>%
    purrr::discard(~all(is.na(.data$.x)))  %>% # drop columns if everything is NA
    dplyr::filter(if_all(everything(), ~ !is.na(.data$.x))) %>%
    ...
    tidyr::unite(.data$condition, -c(.data$row, .data$column, .data$well), sep = "__", remove = FALSE) %>% # make condition column
