...
#' @param .x_vec a string, giving the name of the column containing the x data. Passed to get_drfu_tmas.
#' @param .precision a number, giving the precision by which to iterpolate the drfu values to get drfu tmas. Defaults ot 0.001, which roughly corresponds to one or two decimal places of accuracy in actual temperature, when converted from 0 to 1 normalized temperature used in the fitting.
